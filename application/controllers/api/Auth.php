...
class Auth extends CI_Controller {
    ...
    public function login_post()
    {
        ...
        if(!$this->form_validation->run()){
            $this->response(['status' => false, 'error' => 'Fields Required', 'data' => $this->form_validation->error_array()], 400);
        }else{
            ...
            if($user->num_rows() == 0){
                ...
                if(hash_equals($this->post('password'), $auth->password)){
                    if($auth->aktif != 'Y'){
                        ...
                        $this->response(['status' => true, 'message' => 'Berhasil login. Selamat Datang di SIADIK', 'data' => $session], 200);
                    }
                } else {
